...
export const rule = createRule({
    ...
    meta: {
        type: 'problem',
        docs: {
            description: 'Disallow LLM client imports in crisis or companion routes — these paths must be deterministic',
        },
        messages: {
            noLlmOnCrisisRoute: 'LLM client must not be imported in crisis or companion routes — these paths are deterministic',
        },
        schema: [],
    },
    defaultOptions: [],
    create(context) {
        return {
            ImportDeclaration(node) {
                ...
                const inAppDir = filename.includes('/app/') || filename.startsWith('app/');
                ...
                const isLlmImport = importSource === '@disciplineos/llm-client' ||
...
//# sourceMappingURL=no-llm-on-crisis-route.js.map